...
def default_get(self, fields):
    ...
    for emp in employees:
        ...
        for permiso in permisos:
            ...
            permiso_start = fields.Datetime.from_string(permiso.date_from)
            permiso_end = fields.Datetime.from_string(permiso.date_to)
            first_day_dt = fields.Datetime.from_string(fields.Date.to_string(first_day))
            last_day_dt = fields.Datetime.from_string(fields.Date.to_string(last_day)) + timedelta(days=1)
            
            # Calcular el rango que intersecta con el mes actual
            start_datetime = max(permiso_start, first_day_dt)
            end_datetime = min(permiso_end, last_day_dt)
        ...
        nomina_lines.append((0, 0, {
            ...
            'permisos': round(permisos_horas, 2),  # Redondear a 2 decimales
